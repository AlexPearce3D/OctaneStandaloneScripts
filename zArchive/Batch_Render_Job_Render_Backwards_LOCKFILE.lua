...
function batchRenderJobScript:clampAnimationFrameInputs()
    ...
    local firstFrame = tonumber(self:getInputValue(self.staticInputs["firstFrame"])) or 0
    local lastFrame = tonumber(self:getInputValue(self.staticInputs["lastFrame"])) or 0
...
function batchRenderJobScript:createInputLinkers(graph)
    ...
    local inputInfos =
    {
        ...
        { type = octane.PT_ENUM, label = "Render priority", defaultNodeType = octane.NT_ENUM,
                defaultValue = 1, enum ={ "Per frame", "Per render target",},
        { type = octane.PT_BOOL, label = "Render backwards", defaultNodeType = octane.NT_BOOL,
          defaultValue = false, group = "Animation" }, group = "Animation" }, 
...
function batchRenderJobScript:getRenderTargetIndexAndFrameNumber(resultIx)
    ...
    local renderBackwards = self:getInputValue(self.staticInputs["Render backwards"]) or false
    if renderBackwards then
        frameIx = lastFrame - (frameIx - firstFrame)
    end
...
function batchRenderJobScript.onIterate(self, graph, resultIx)
   ...
   local totalResultFrames = self:calcTotalResultFrames();
    ...
    -- Create placeholder lock file to prevent duplicate frame renders
    local outputDirectory = self:getOutputDirectory(graph)
    ...
    if lockFile then
        lockFile:write("Claimed by render node")
        lockFile:close()
    end
